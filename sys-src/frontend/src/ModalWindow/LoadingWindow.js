...
export default function LoadingWindow({ open, onClose }) {
  ...
  }, [animationSettings.positionX, animationSettings.positionY, animationSettings.velocityX, animationSettings.velocityY]);
  ...
  return (
    <Dialog open={open} maxWidth="xs" fullWidth>
      ...
      <DialogContent dividers sx={{ width: BOUNCINGWIDTH, height: BOUNCINGHEIGHT, padding: 0 }}>
          <CardMedia src={cat} component="img" alt="Loading" sx={{ width: `${IMAGESIZE}px`, objectFit: "contain", position: "absolute", transform: `translate(${animationSettings.positionX}px, ${animationSettings.positionY}px)` }} />
      </DialogContent>
    </Dialog>
  );
}