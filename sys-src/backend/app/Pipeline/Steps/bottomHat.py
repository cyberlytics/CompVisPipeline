    ...
    def __call__(self, img, parameters):
        try:
            ...
            if len(img.shape) not in (2, 3): 
                raise WrongParameterError(message="[Bottom Hat] Invalid image shape.")
            if kernel_shape not in {0, 1, 2}: 
                raise WrongParameterError(message="[Bottom Hat] Invalid kernel shape.")
            if kernel_width < 2: 
                raise WrongParameterError(message="[Bottom Hat] Kernel width must be bigger than 1.")
            if kernel_height < 2: 
                raise WrongParameterError(message="[Bottom Hat] Kernel height must be bigger than 1.")
            if iterations < 1: 
                raise WrongParameterError(message="[Bottom Hat] Iterations must be bigger than 0.")
            ...
            raise WrongParameterError(message=f"[Bottom Hat] {e}")

        except Exception as e:
            raise ImageProcessingError(message=f"[Bottom Hat] {e}")
